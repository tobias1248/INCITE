...
class ConcolicTestRecorder:
    ...
    def first_execution_end(self):
        # iteration 0 only generates constraints; solve time = 0
    ...
    def save_adversarial_input_as_image(self, save_path):
        if self.adversarial_input is not None:
            img_0_255 = (self.adversarial_input * 255).astype(np.uint8)
    ...
    def _summarize_numeric(values):
        if not values:
            return None
        total = sum(values)
        return {"min": min(values), "max": max(values), "mean": total / len(values), "sum": total}
    ...
    def _build_progress(self):
        ...
        if ton_current is None and ton_next is None:
            return None
        return {"ton_current": ton_current, "ton_next": ton_next, "stop_at": None, "reason": None}
    ...
    def output_stats_dict(self, constraint_complexity=None):
        ...
        summary["execute_wall_time_total"] = sum(self.execute_wall_time) if self.execute_wall_time else None
        summary["execute_cpu_time_total"] = sum(self.execute_cpu_time) if self.execute_cpu_time else None
        ...
        summary["iter_wall_time_total"] = sum(self.iter_wall_time) if self.iter_wall_time else None
        summary["iter_cpu_time_total"] = sum(self.iter_cpu_time) if self.iter_cpu_time else None
    ...
    def save_stats_dict(self, constraint_complexity=None):
        ...
        with open(os.path.join(self.save_dir, "stats.json"), "w") as f:
            json.dump(stats_dict, f, default=self._json_safe)
        ...
        if self.original_input is not None:
            np.save(os.path.join(self.save_dir, "ori_input.npy"), self.original_input.astype(np.float32, copy=False))
        if self.adversarial_input is not None:
            np.save(os.path.join(self.save_dir, "adv_input.npy"), self.adversarial_input.astype(np.float32, copy=False))

        if len(self.sat_inputs) == 0:
            np.save(
                os.path.join(self.save_dir, "sat_inputs.npy"),
