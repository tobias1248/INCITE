...
class ShapValuesComparator:
    ...
    def calculate_shap_values_for_all_neurons(self) -> None:
        ...
        with open(file_path, "w") as json_file:
            json.dump(self.shap_values, json_file, indent=2)
    ...
    def calculate_shap_values(
        ...
        if self.explainer_type == "gradient":
            ...
            print("type:", type(background_dataset))
            print("len:", len(background_dataset) if isinstance(background_dataset, list) else "N/A")
            print("shape:", background_dataset.shape if isinstance(background_dataset, np.ndarray) else "N/A")
            print("========================")
            ...
            print("------shap_values------", type(shap_values), len(shap_values), shap_values[0].shape)

            
            shap_values_summed = np.sum(shap_values, axis=1)
            # print(shap_values_summed.shape, type(shap_values_summed), len(shap_values_summed))
            average_shap_values = np.squeeze(np.mean(shap_values_summed, axis=0))
            # print(average_shap_values.shape, type(average_shap_values))
