...
class FakeModel:
    def __call__(self, batch, training=False):
        ...
        for sample in batch:
            ...
            if np.isclose(marker, 0.1):
                outputs.append([0.9, 0.1, 0.0])
            elif np.isclose(marker, 0.8) or np.isclose(marker, 20.0):
                outputs.append([0.1, 0.8, 0.1])
            elif np.isclose(marker, 0.9):
...
def test_replay_adversarial_cases_filters_success_cases_and_flags_mismatches(tmp_path, monkeypatch) -> None:
    ...
    assert records[1].input_range_valid is True
...
def test_main_json_output_and_fail_on_issues(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert payload["valid_adversarial_count"] == 1
    assert payload["input_range_invalid_count"] == 0
    assert payload["original_prediction_mismatch_count"] == 1


def test_main_fails_on_out_of_range_adversarial_input(tmp_path, monkeypatch, capsys) -> None:
    exp_root = tmp_path / "exp"
    _write_case(
        exp_root,
        "case_0",
