...
import tasks.builders.mnist as builders


class _DummyMnistDataset:
    ...
    def get_mnist_test_data_and_set_condict(self, idx, attack_pixels):
        in_dict, con_dict, input_for_shap, background = self.get_mnist_test_data(idx)
        for i, j, k in attack_pixels:
            con_dict[f'v_{i}_{j}_{k}'] = 1
        return in_dict, con_dict, input_for_shap, background
...
class _DummyCalculator:
    ...
    def ensure(self, assume_cached: bool, force_refresh: bool) -> None:
        ...
        self.last_timing = {
            ...
            "compute_seconds": float(self.kwargs['idx']) + 0.5,
        }


def test_mnist_transformer_shap_builds_ton_plans(monkeypatch) -> None:
    monkeypatch.setattr(builders, 'MnistDataset', _DummyMnistDataset)
    monkeypatch.setattr(builders, 'JsonShapPixelProvider', _DummyPixelProvider)

    inputs = builders.mnist_transformer_shap('demo', [0], force=True, ton_values=(1, 2))
...
def test_mnist_transformer_random_uses_coordinate_provider(monkeypatch) -> None:
    monkeypatch.setattr(builders, 'MnistDataset', _DummyMnistDataset)
    monkeypatch.setattr(builders, 'make_coordinate_provider', lambda *args, **kwargs: (lambda idx, ton: [(1, 1, 0)] * ton))

    inputs = builders.mnist_transformer_random('demo', [1], ton_values=(1,), force=True)

    assert len(inputs) == 1
    assert inputs[0]['ton_plans'][0]['con_dict'] == {'v_1_1_0': 1}


def test_mnist_transformer_shap_calculate_all_runs_calculator(monkeypatch, tmp_path) -> None:
    _DummyCalculator.calls = []
    monkeypatch.setattr(builders, 'MnistDataset', _DummyMnistDataset)
    monkeypatch.setattr(builders, 'ShapValuesCalculator', _DummyCalculator)

    artifacts = builders.mnist_transformer_shap_calculate_all(
        'demo',
        first_n_img=2,
        force_refresh=True,
    ...
    assert [item['compute_seconds'] for item in artifacts] == [0.5, 1.5]
